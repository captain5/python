...
# change StringIO to io
# change reload() to imp.reload()

__author__ = 'captain5 zhang'
from bs4 import BeautifulSoup
import urllib.request,sys,io,gzip,time,random,re,os,imp
imp.reload(sys)
...
class Xitek():
    ...
    def __download(self,url):
        ...
        html=str(self.__getContentAuto(url))
        ...
        for i in content:
            ...
            urllib.request.urlretrieve(final_link,filename)
            #f=open(filename,'w')
            #f.write(pic_stream)
            #f.close()
        #print html
